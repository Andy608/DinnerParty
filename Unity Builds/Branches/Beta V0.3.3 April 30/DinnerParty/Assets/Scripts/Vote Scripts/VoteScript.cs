...
//    private List<Button> mPlayerMeals;
    ...
    public void OnContinueClicked()
    {
		if(mVotingIsHappening){
			...
			gameObject.GetComponent<VoteTimerScript> ().TurnOnTimer ();
		}
		else
    ...
    private void PlacePlayersInCircle()
    {
        ...
        //Scale radius by screen size to keep it consistent.
        float radius = mCanvas.pixelRect.width / (3.0f + (scale - 1.0f));
        //float radius = mCanvas.pixelRect.width / 3.0f;
    ...
    /*private void PlacePlatesInCircle()
    {
        ...
        //Scale radius by screen size to keep it consistent.
        float radius = mCanvas.pixelRect.width / (3.0f + (scale - 1.2f));

        int i;
        for (i = 0; i < players.Count; ++i)
        {
            Button userPlate = Instantiate(mPlatePrefab, mTableCenter.transform);
            userPlate.transform.localScale = new Vector3(scale, scale, scale);
	...
	public void ActivateAllMeals()
	{
		tieButton.interactable = true;

		for (int i = 0; i < mPlayerNamecards.Count; i++)
			mPlayerNamecards [i].interactable = true;
	}

	public void DeactivateAllMeals()
	{
		tieButton.interactable = false;

		for (int i = 0; i < mPlayerNamecards.Count; i++)
			mPlayerNamecards [i].interactable = false;
	}
}
