...
public class ShowRoleScript : MonoBehaviour
{
	...
	{
        //I used 1.5 instead of 2 so there would be a little more room around the window border.
        int widthPadding = (int)(mActionButton.GetComponent<RectTransform>().rect.width / 1.5);
        int heightPadding = (int)(mActionButton.GetComponent<RectTransform>().rect.height / 1.5);

        int randomX = Random.Range (0 + widthPadding, Screen.width - widthPadding);
		int randomY = Random.Range (0 + heightPadding, Screen.height - heightPadding);
