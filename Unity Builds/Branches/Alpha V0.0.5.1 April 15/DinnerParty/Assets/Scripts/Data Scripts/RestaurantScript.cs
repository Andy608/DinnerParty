...
public class RestaurantScript : MonoBehaviour
{
    ...
    public void resetPlayersWithTurnsLeft()
    {
	...
	private void InvestigatePlate(Player clickedMeal)
	{
		...
		if (mMeals [clicked].isSpecial() && mSelectedPlayer.getRole() == EnumPlayerRole.FOOD_CRITIC)
			Debug.Log ("Special Type: " + mMeals [clicked].getTypeOfSpecialMeal().ToString());

		if (mSelectedPlayer.getRole() == EnumPlayerRole.CHEMIST)
