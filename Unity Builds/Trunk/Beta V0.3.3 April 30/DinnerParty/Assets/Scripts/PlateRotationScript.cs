...
public class PlateRotationScript : MonoBehaviour {
	...
	void OnMouseUp()
	{
		...
		if (mIsRotated) {
			if (mNumRotated >= 3 || mNumRotated <= -3) {
				GameManagerScript.GetInstance ().GetComponent<RestaurantScript> ().PoisonPassPanel (mNumRotated);
			}
			else if (mNumRotated != 0)
			//if (mNumRotated != 0)
			{
				GameManagerScript.GetInstance ().GetComponent<RestaurantScript> ().EndWhoseTurn ();
			}
		}
