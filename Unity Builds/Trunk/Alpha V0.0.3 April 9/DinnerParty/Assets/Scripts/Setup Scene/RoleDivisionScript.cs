...
public class RoleDivisionScript : MonoBehaviour
{
	...
	public List<PlayerScript> mListOfPlayers;
	...
	bool RandomizeRoles()
	{
		if (mNumberOfPlayers == mListOfRoles.Count) {
			for (int i = 0; i < mNumberOfPlayers; i++) {
				...
				PlayerScript newPlayer = new PlayerScript(currentPlayerName);
			...
			for (int j = 0; j < mListOfRoles.Count; j++) {
				...
				} while(mListOfPlayers [playerNumber].isRoleFound());
							
				PlayerScript tempPlayer = mListOfPlayers [playerNumber];
